...
//-------------------------------------------------------------------------------------------------
void HEPHero::SetupSignalPoints() {
    ...
    //_outputTree->Branch("variable1NameInTheTree", &SignalPoints::variable1Name );  [example]

    //======SETUP INFORMATION IN OUTPUT HDF5 FILE==================================================
    //HDF_insert("variable1NameInTheTree", &SignalPoints::variable1Name );  [example]


    HDF_insert("FatJet_msoftdrop", &SignalPoints::FatJet_msoftdrop);
    HDF_insert("PFMET_pt",&PFMET_pt);
    HDF_insert("MHT",&MHT);
...
//-------------------------------------------------------------------------------------------------
bool HEPHero::SignalPointsRegion() {
...
//      if ( !(signal_tag>0.8) ) return false;
	...
	if (selectedFatJet.size()>0){
        	SignalPoints::FatJet_pt = FatJet_pt[selectedFatJet.at(0)];
        	SignalPoints::FatJet_msoftdrop = FatJet_msoftdrop[selectedFatJet.at(0)];
	}
	if (selectedFatJet.size()==0){
		SignalPoints::FatJet_pt = 0;
                SignalPoints::FatJet_msoftdrop = -1;
	}
...
//-------------------------------------------------------------------------------------------------
void HEPHero::SignalPointsSelection() {
    ...
    //SignalPoints::variable1Name = 100;      [Example]
...
//-------------------------------------------------------------------------------------------------
void HEPHero::SignalPointsSystematic() {
...
//-------------------------------------------------------------------------------------------------
void HEPHero::FinishSignalPoints() {
